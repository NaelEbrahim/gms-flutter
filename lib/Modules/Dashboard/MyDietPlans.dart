...
class MyDietPlans extends StatelessWidget {
  Manager? _manager;

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => Manager()..userDietPlans(),
              ...
              builder: (context) {
                if (state is SuccessState) {
                  return ListView.builder(
                    ...
                    itemCount: _manager?.userDiets.length,
                    itemBuilder: (context, index) {
                      final item = _manager?.userDiets.elementAt(index);
            ...
            Row(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    item.isActive.toString() == 'true' ? 'Active' : 'Inactive',
                    style: const TextStyle(
                      color: Colors.black,
                      fontSize: 12,
                      fontWeight: FontWeight.bold,
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        ReusableComponents.deleteDialog<Manager>(
                          context,
                          () async {
                            _manager?.deleteDietFeedback(item.id.toString());
                          },
                        );
                      },
                    ),
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: Text("Save"),
            onPressed: () {
              _manager?.updateDietRate({'dietId': item.id, 'rate': tempRate});
            },
          ),
        ],
      ),
  ...
  void _showFeedbackDialog(BuildContext context, DietPlanModel item) {
    final controller = TextEditingController(text: item.myFeedback);
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        backgroundColor: const Color(0xff2a2a2a),
        title: Text(
          item.myFeedback == null ? "Add Feedback" : "Edit Feedback",
